...
void groupByKey(IntermediateInput *input, Output *output, int *result_count) {
  ...
  Output *endarr = output + *result_count;

  for (Output *res = output; res < endarr; ++res) {
    if (res->doubled_value == input->doubled_value) {
      res->line_numbers[res->count++] = input->line_number;
      return;
    }
  }

  endarr->doubled_value = input->doubled_value;
  endarr->count = 0;
  endarr->line_numbers[endarr->count++] = input->line_number;
  (*result_count)++;
}
...
void reduce(Output *output) {
  ...
  for (int i = 0; i < output->count; i++) {
    printf("%s%d", separator, output->line_numbers[i]);
    separator = ", ";
  }
  printf("])\n");
}
